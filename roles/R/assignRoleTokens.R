  ...
  if(exists("tmp")){ ## if there are some worth checking twice
    if(nrow(tmp)>0){
      ...
      if(length(secondGuess>0)){
        ...
        ## string distance matrix
        distMat <- stringdist::stringdistmatrix(a = second,b=prior$from,method = "jw",p=0.025,nthread = cores)
        help <- apply(distMat, 1,function(x){
          if(min(x) < thresh2){ 
            return(prior$org_trans[which.min(x)])
          }else{
        ...
        ii2 <- na.omit(ii2)
        if(nrow(ii2)>0){
          ujemanje3 <- match(ii2$org_trans,prior$org_trans)
          ii2 <- cbind(from=ii2$from,prior[ujemanje3,5:ncol(prior)],stringsAsFactors = F)
