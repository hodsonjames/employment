...
dataset_names = [x for x in os.listdir('../cognism-enriched/') if 'done' not in x]



# process one year of data and produce a page rank by iterating through all datasets for one year
def process_one_year(year, batch_size, output_path):
	# load the yearly graph
	for dataset in dataset_names:
		# absolute path of the dataset
		file_path = 'file:///DATA-0/fedyk/cognism-enriched/' + dataset
		...
		# if an egde already exits, will this set the weight of already existing edge to 0?
		query = """
				MATCH (e1:Employee)-->(c1:Company), (e2:Employee {eid: e1.eid})-->(c2:Company) 
				WHERE e1.edate < e2.sdate
				MERGE (c1)-[t: TRANS {weight:0}]->(c2)
			"""
...
for year in years:
	process_one_year(year, batch_size, output_path)